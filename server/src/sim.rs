...
// `_grid`/`_occupied`/`_conveyor_running`: 이 태스크 시점엔 Assembly/Helper
// 둘 다 스스로 이동하지 않으므로 쓰이지 않는다(위 각 분기의 doc 주석
// 참고) — Task 3가 헬퍼 이동 로직을 채우면 다시 쓰이게 되므로 시그니처는
// 미리 맞춰두고 이름만 밑줄로 죽은 파라미터 경고를 끈다.
fn plan_robot(
    _grid: &Grid,
    robot: &Robot,
