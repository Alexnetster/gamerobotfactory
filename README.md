...
# 테스트 전체 실행 (84개, 인테그레이션 테스트 일부는 실제 서버 바이너리를 띄워 수 초 소요)
...
| `GET /metrics` | Prometheus 텍스트 포맷(`ticks_total`, `connected_clients`, `robot_count`, `tick_panics_total`) |
...
- **Plan 5**: 데모/배포 (Docker, 라이브 URL, 성능 목표 실측 — 틱 처리시간 p99 메트릭은 아직 없어 Plan 5 전에 추가 검토 필요).
