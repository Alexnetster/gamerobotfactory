...
**현재: 서버 151개 테스트 통과 + 클라이언트 vitest 단위 64개/통합 3개, Playwright E2E 3개, `node --test` 성능 스크립트 파싱 테스트 4개 통과. clippy(`--all-targets`, `-D warnings` 없이)는 이 플랜과 무관한 기존 unknown-lint 경고 1건(`sim.rs:363`, 상세는 KANBAN Backlog)만 남고 그 외 0개 — `-D warnings`로 돌리면 그 알림이 컴파일 에러로 승격돼 빌드 자체가 실패한다.**
