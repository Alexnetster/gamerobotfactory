...
일자형 벨트를 따라 드론(제품)이 흐르며 조립되는 공장 시뮬레이션. 4족 조립 로봇 3대가 각자 고정된 스테이션 옆에서 절대 이동하지 않고 제품을 단계별로 조립하고(빈 프레임 → 배터리 → 프로펠러 → 완성), 헬퍼 로봇(수 조절 가능)이 창고에서 부품/프레임을 실어 날라 라인이 멈추지 않게 한다. **백엔드/서버 설계 역량을 보여주는 포트폴리오 프로젝트**로, 게임성은 2차 목표다 — 결정적 시뮬레이션 코어, WebSocket 델타 동기화 프로토콜, 동시성 안전한 틱 루프가 이 프로젝트의 실제 핵심이다.

(예전에는 로봇들이 목적 없이 자유이동하며 아무 칸에서나 픽업/배치하는 모델이었다 — "로봇 수를 늘렸는데 왜 더 안 늘어난 것처럼 보이냐"는 실사용 혼란을 겪은 뒤, 조립 로봇은 항상 고정 3대 + 사용자가 조절하는 건 헬퍼 로봇 수뿐이라는 지금의 일자형 조립 라인 구조로 완전히 대체했다. 배경은 [`docs/superpowers/specs/2026-07-24-assembly-line-design.md`](docs/superpowers/specs/2026-07-24-assembly-line-design.md) 참고.)
...
sim_core (라이브러리, 네트워크 의존성 없음)
  ...
  Product/Station/RobotRole(조립 로봇 고정, 헬퍼 작업 큐/배정) · 일자형 벨트 전진+조립 로직

server (바이너리, sim_core에 의존)
  GameState(컨베이어/헬퍼 로봇수/선택/팔동작) · JSON 와이어 프로토콜(버전 필드,
...
# 서버 테스트 전체 실행 (168개, 인테그레이션 테스트 일부는 실제 서버 바이너리를 띄워 수 초 소요)
...
| `ReleaseRobot` | — | 선택 해제 |
| `ToggleConveyor` | — | 컨베이어(=라인 전체) on/off |
| `SetRobotCount` | `count: number` | **헬퍼** 로봇 수 조절(1~200으로 클램프). 조립 로봇 3대는 항상 존재하며 이 커맨드로 늘리거나 줄일 수 없다 |
| `TriggerArmAction` | `robot_id: number, task: "Idle"\|"Picking"\|"Placing"` | 팔 동작 트리거(레거시 — 조립/헬퍼 로봇 모두 매 틱 실제 작업 상태로 `task`가 덮어써지므로 사실상 무력화됨, 상세는 `docs/KANBAN.md` Backlog) |
| `RepairRobot` | `robot_id: number` | 고장난(`Failed`) 로봇 수리 시작 (오퍼레이터 트리거) |
| `RepairAllRobots` | — | 현재 고장난 로봇 전부 일괄 수리 시작(대상이 없어도 오류 아님) |
...
| 메시지 | 필드 | 설명 |
|---|---|---|
| `Snapshot` | `v, tick, session_id, conveyor, robots[], stations[], products[]` | 최초 접속 시 + Lagged 리싱크 시 전체 상태 |
| `Delta` | `v, tick, conveyor?, changed_robots[], removed_robot_ids[], stations[], changed_products[], removed_product_ids[]` | 매 틱, 바뀐 것만(`stations[]`는 재고 변화가 잦아 압축하지 않고 매번 전체를 실음) |
...
- `role` — `{"kind":"Assembly","station_index":N}`(N번 스테이션에 고정, 절대 이동 안 함) 또는 `{"kind":"Helper"}`(창고↔스테이션/라인 시작점을 오가며 재고 보충·프레임 배달).
- `carrying` — 헬퍼가 지금 화물(부품/프레임)을 들고 이동 중인지. 조립 로봇에서는 항상 `false`.
- `path` — 현재 A* 경로(칸 좌표 배열, 헬퍼 이동 시에만 채워짐). 렌더링에서는 디버그 오버레이용, 실제 이동은 서버가 이미 계산해 `pos`로 반영한다.
...
`stations[]`의 각 항목(`StationView`)은 `index`(0~2, `station_index`/제품 `stage`와 대응), `robot_cell`(그 옆에 고정된 조립 로봇 위치), `part_inventory`(남은 부품 재고, 0이면 그 스테이션은 헬퍼가 보충할 때까지 대기)를 담는다.

`products[]`/`changed_products[]`의 각 항목(`ProductView`)은 벨트를 흐르는 드론 하나를 나타낸다 — `id`, `stage`(0=빈 프레임, 3=완성 후 벨트 끝에서 반출), `pos`.
...
브라우저에서 `http://localhost:5173/?ws=ws://127.0.0.1:54321/ws`를 열면(포트 번호는 위 1번 단계에서 실제로 뜬 `LISTENING_PORT` 값으로 교체) 아이소메트릭 캔버스와 우측 사이드바가 뜬다. 접속하자마자 조립 로봇 3대가 각자 스테이션 옆에 고정된 채 보이고(사용자가 늘리거나 줄일 수 없음), 사이드바의 `+`/`-`는 **헬퍼 로봇** 수만 조절한다(최소 1대). 컨베이어(=라인 전체) on/off 토글, 캔버스에서 로봇을 클릭해 선택 후 고장(`Failed`)난 로봇은 `수리` 버튼으로 복구할 수 있다 — 그 옆의 `Picking`/`Placing`/`Idle` 버튼은 예전 자유이동 모델의 흔적으로, 지금은 두 로봇 역할 모두 매 틱 실제 작업 상태가 `task`를 덮어써서 사실상 아무 효과가 없다(`docs/KANBAN.md` Backlog 참고). 컨베이어를 켜두면 헬퍼가 창고에서 빈 프레임을 실어와 벨트에 올리고, 그 프레임이 스테이션 3개를 순서대로 거치며 조립(빈 프레임 → 배터리 → 프로펠러 → 완성 체크)된 뒤 벨트 끝에서 반출된다 — 스테이션 재고가 바닥나면(로봇 머리 위 빨간 경고 삼각형) 헬퍼가 알아서 보충하러 간다.

**페이지가 응답이 없거나 접속이 안 될 때**: 서버(`cargo run`) 또는 클라이언트(`npm run dev`) 터미널 중 하나가 꺼져 있는 게 원인인 경우가 대부분이다(터미널 창을 닫았거나, 컴퓨터가 절전 모드에 들어갔거나, 둘 중 하나가 크래시). 확인 방법:

1. 두 터미널 창이 아직 열려 있고 프로세스가 살아있는지 본다 — 서버 터미널은 `LISTENING_PORT=...` 줄 이후 계속 떠 있어야 하고, 클라이언트 터미널은 `VITE ... ready` 이후 계속 떠 있어야 한다. 프롬프트로 돌아가 있으면(`$` 또는 `>`만 있으면) 죽은 것이다.
2. 죽어있는 쪽을 위 1)/2) 명령으로 다시 실행한다. **포트는 재실행할 때마다 다시 무작위로 배정되므로**(서버는 `LISTENING_PORT=`, 클라이언트는 `Local:` 줄), 이전에 쓰던 주소를 그대로 재사용하지 말고 새로 뜬 포트로 URL을 다시 만들어 접속한다.
3. 서버만 재시작했다면 클라이언트 브라우저 탭은 그대로 두고 주소창의 `ws=` 뒷부분(포트)만 새 값으로 바꿔서 다시 열면 된다. 클라이언트(`npm run dev`)를 재시작했다면 브라우저 탭도 새로고침해야 한다(Vite 개발 서버가 꺼지면 이미 열려있던 탭은 페이지 자체가 새로 안 뜬다).
...
# → 접속 즉시 {"kind":"Snapshot", ..., "robots":[...3대...], "stations":[...], "products":[]}
#   수신 (조립 로봇 3대는 항상 이미 존재, 아직 제품은 0개)

# 3) 헬퍼 로봇 2마리 배치
> {"type":"SetRobotCount","count":2}
# → 다음 틱부터 {"kind":"Delta", "changed_robots":[...]} 로 role.kind=="Helper"인
#   로봇 2마리가 새로 등장(조립 로봇 3대는 안 바뀌었으므로 델타에 안 실림)

# 4) 컨베이어(=라인 전체)를 켠다 — 헬퍼가 창고에서 프레임을 실어와 벨트에 올리고
#    스테이션 3개를 거치며 조립되는 걸 몇 초 안에 관찰할 수 있다
> {"type":"ToggleConveyor"}

# 5) 연결을 끊었다가(Ctrl+C) 30초 안에 재접속해 세션 이어가기
wscat -c ws://127.0.0.1:54321/ws
> {"type":"Resume","session_id":"<2)에서 받은 session_id>"}
...
- **조립 라인(일자형 벨트 + 스테이션 + 헬퍼 로봇)**: "로봇 수를 늘렸는데 왜 화면엔 그대로인 것 같냐"는 실사용 혼란(자유이동 모델에서는 로봇이 전부 화면 여기저기 흩어져 있어 몇 대가 실제로 늘었는지 체감이 안 됐다) → 브레인스토밍 → 설계(독립 리뷰어 완결성 리뷰로 스테이션 좌표 충돌, 헬퍼 유휴 스폰 위치가 활성 헬퍼의 목적지를 영구 점거하는 배포 정지 버그, `tick()` 시그니처 확장 시 기존 30여 개 호출부 회귀 위험을 미리 발견해 스펙에 반영) → 구현 계획(10태스크) → subagent-driven-development로 태스크별 구현자→스펙 리뷰어→코드 품질 리뷰어 루프 실행, 총 10개 태스크 전부 완료. 자유이동 픽업→운반→배치 사이클(`work_points`/U자 `belt_cells`/순찰)을 통째로 제거하고, 조립 로봇 3대(항상 존재, 사용자가 조절 불가)가 각자 고정 스테이션 옆에서 제품을 단계별로 조립(0=빈 프레임→1=배터리→2=프로펠러→3=완성)하고, 헬퍼 로봇(수 조절 가능, 최소 1대)이 창고↔스테이션/라인 시작점 사이를 오가며 재고 보충·프레임 배달을 담당하는 구조로 완전히 대체했다. 제품/스테이션 이동도 로봇과 똑같은 이중버퍼+ID 타이브레이크 규칙을 그대로 재사용해서(`MoveIntent`/`resolve_intents`를 로봇/제품 공용으로 리네이밍) 결정성을 유지했다. 실제 개발 중 실측으로 발견/수정한 버그 3건: (1) 헬퍼가 새로 스폰될 때 전부 같은 창고 칸에 서 있으면, 활성 헬퍼가 픽업하러 그 칸에 도착하기 직전 마지막 한 칸이 영원히 안 열려 라인 전체가 멈추는 배포 정지 버그(라이브 서버로 실측 재현 후 스폰 위치를 6개 대기 칸으로 분산시켜 해소) — 또 다른 헬퍼 관련 교착 하나도 같은 방식(실제 서버+WS 클라이언트로 재현)으로 잡아 고쳤다. (2) 클라이언트가 새로 그리는 제품 배터리 스프라이트가 로봇이 화물을 들었을 때 그리는 기존 화물 아이콘과 우연히 정확히 같은 색(`#c9762f`)을 써서, 실제 캔버스 렌더링을 검증하던 Playwright E2E 테스트가 무관한 제품/다른 로봇 때문에 오탐하던 것을 전체 회귀 검증(신선한 서버로 8회 연속 실행) 중 발견해 색을 분리하고, "이번에 새로 스폰한 로봇이 실제로 carrying인지"는 서버 권위 상태(병렬 WS 연결)로 확정한 뒤 렌더링 자체는 공용 `drawRobot` 코드 경로를 근거로 검증하도록 테스트를 재작성. (3) 통합테스트들의 "초기 로봇 0마리"/"SetRobotCount=총 로봇 수" 가정이 "조립 로봇 3대는 항상 존재"라는 새 모델과 어긋나 있던 걸 찾아 전부 갱신. 클라이언트는 일자 벨트 렌더링, 제품 스프라이트(stage별 부품 추가), 스테이션 재고 0일 때 경고 삼각형, 사이드바의 "헬퍼 로봇 수" 라벨 재조정까지 반영.

**현재: 서버 168개 테스트 통과 + 클라이언트 vitest 단위 77개/통합 3개, Playwright E2E 3개, `node --test` 성능 스크립트 파싱 테스트 4개 통과. `cargo clippy --all-targets -- -D warnings` 경고 0개(조립 라인 작업 중 이전까지 남아있던 Rust 버전 스큐발 unknown-lint 경고도 관련 코드가 함께 정리되며 자연히 해소됨).**
