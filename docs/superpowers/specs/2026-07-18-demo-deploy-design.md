...
이 변경은 "목표에 도착하면 정지한다"는 옛 의미를 "목표(순찰 지점)에 도착하면 다음 순찰 지점으로 향한다"로 바꾸므로, 그 옛 의미를 직접 검증하던 기존 테스트 3개가 새 동작과 충돌한다:
...
- `facing_holds_last_direction_while_stationary`(`server/src/sim.rs:601-625`) — 계획 작성 중 직접 손으로 추적해서 발견한 세 번째 충돌. 이 테스트는 마지막 구간에서 `robot.goal`을 로봇이 이미 도착한 위치(`settled`)로 수동 대입해 "정지 상태에서도 facing이 안 바뀐다"를 확인한다. 그런데 `settled`는 그 로봇의 순찰 지점 A/B 중 어느 것도 아니라서(테스트가 임의로 서쪽 목표(0,0)로 goal을 직접 조작했었기 때문), `next_patrol_goal`이 무조건 A로 재배정하고 그 A가 동쪽에 있어 로봇이 다시 움직이며 facing이 East로 바뀐다 — 원래 기대값(West 유지)이 깨진다. 이 테스트가 검증하려는 진짜 불변식("완전히 멈춘 로봇은 facing을 유지한다")은 여전히 유효하지만, "도착 = 정지"라는 낡은 전제가 깨졌으므로 정지를 보장하는 다른 수단이 필요하다 — `RobotStatus::Failed`로 만들면 `plan_robot`이 이동/순찰 재배정을 포함해 전부 건너뛰므로, 마지막 단계를 `goal` 대입 대신 `status = RobotStatus::Failed`로 바꿔 재작성한다.

세 테스트 모두 "제자리 정지"가 아니라 "순찰 목표 재배정"(혹은 진짜 정지 수단인 Failed 상태)을 검증하도록 재작성한다(아래 구현 계획 참고) — 낡은 assertion을 그냥 지우는 게 아니라, 바뀐 의도를 정확히 표현하는 새 assertion으로 바꾸는 것.
