...
>
> **갱신 규칙**: 태스크를 시작하면 Backlog → In Progress로, 리뷰(spec+code quality)까지 통과하면 In Progress → Done으로 옮긴다. Done 항목에는 커밋 SHA를 남긴다.
...
- **Task 7** — WebSocket 핸들러 (초기 스냅샷 + 커맨드 적용) (`2e25d1b`) — 실제 WS 클라이언트로 구현자·리뷰어 각자 독립 검증됨

## In Progress

- **Plan 2 Task 7 후속 수정**: `ws.rs`의 `socket.recv()`가 `Some(Err(_))`를 반환하는 경우 다른 거부 경로와 달리 로그를 안 남기고 조용히 루프 종료 — 사소하지만 파일의 "모든 거부는 로깅한다" 일관성에서 벗어남. 반영 여부/타이밍 결정 대기 중(Task 8과 묶어서 처리할 수도 있음).

## Backlog

### Plan 2 나머지
- **Task 8** — 20Hz 틱 루프 + 델타 브로드캐스트 (`compute_delta`를 실제로 연결 — 현재 clippy의 유일한 경고인 unused 경고가 여기서 해소됨). 리뷰 체크포인트: 락을 잡은 채로 소켓 send를 await하지 않는다는 원칙(Task 7에서 확인됨)이 깨지지 않는지.
- **Task 9** — 세션/재접속 유예시간 (순수 로직은 계획에 이미 있음, 실제 소켓 배선은 스트레치)
- **Task 10** — 통합테스트 (실제 서버 바이너리 + `tokio-tungstenite` 클라이언트, OS 임의 포트로 충돌 방지)

### Plan 3~5 (아직 계획 문서 없음, 설계문서 로드맵만 있음)
- **Plan 3** — 영속화(SQLite) + REST API + 관측가능성(`/metrics`, tracing)
...
- `cargo test --manifest-path server/Cargo.toml`: 58/58 통과
- `cargo clippy --all-targets`: 경고 1개(`compute_delta` unused, Task 8에서 해소 예정)
